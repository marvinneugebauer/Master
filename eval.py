...
class Dataloader:
    ...
    def image_loader(self, path_to_pred, path_to_gt):
        """
         ...
         loader function updates 4 arrays: The first array self.prediction_images_names contains the names of the
         binarized prediction images. The second array self.prediction_images contains binarized prediction images.
         The third array self.gt_images contains the binarized ground truth images. The fourth array
...
class Evaluate:
    def __init__(self, pred_semseg_array, pred_gt_array):
        """
        We initialize all instances that we need for our calculation of each metric.
        """
        self.pred_semseg_array = pred_semseg_array
        self.pred_gt_array = pred_gt_array
        self.length = len(pred_gt_array)

        self.iou_add = 0
        self.iou_foregrd_add = 0
        self.iou_backgrd_add = 0
        self.mean_iou_add = 0
        self.pixel_acc = 0
        self.intsec_cl1_total = 0
        self.intsec_cl0_total = 0
        self.gt_cl1_total = 0
        self.gt_cl0_total = 0
        self.union_cl1_total = 0
        self.union_cl0_total = 0
        self.false_pos_cl1_total = 0
        self.false_pos_cl0_total = 0
        self.false_neg_cl1_total = 0
        self.false_neg_cl0_total = 0
        self.ignore_pred_total = 0
        self.ignore_gt_total = 0

        self.intersection_class_1 = 0
        self.intersection_class_0 = 0
        self.union_sem_class_1 = 0
        self.union_sem_class_0 = 0
        self.gt_bin_class_1 = 0
        self.gt_bin_class_0 = 0

        self.iou_average = 0
        self.mean_iou_average = 0
        self.iou_foregrd_average = 0
        self.iou_backgrd_average = 0
        self.iou_total = 0
        self.mean_iou_aLL_total = 0
        self.iou_foregrd_total = 0
        self.iou_backgrd_total = 0
        self.pixel_accuracy_average = 0
        self.pixel_accuracy_total = 0
        self.true_pos_foregrd_average = 0
        self.true_pos_backgrd_average = 0
        self.true_pos_average = 0
        self.true_pos_total = 0
        self.false_pos_foregrd_average = 0
        self.false_pos_backgrd_average = 0
        self.false_pos_average = 0
        self.false_pos_total = 0
        self.false_neg_foregrd_average = 0
        self.false_neg_backgrd_average = 0
    ...
    def calculate_iou(self):
        ...
        """
        if len(self.union_sem_class_1) + len(self.union_sem_class_0) == 0:
            return 0
        else:
            iou = (len(self.intersection_class_1) + len(self.intersection_class_0)) / (len(self.union_sem_class_1) +
                                                                                       len(self.union_sem_class_0))
            return iou
    ...
    def calculate_iou_foreground(self):
        ...
        """
        if len(self.union_sem_class_1) == 0:
            return 0
        else:
            iou_foreground = (len(self.intersection_class_1) / len(self.union_sem_class_1))
    ...
    def calculate_iou_background(self):
        ...
        """
        if len(self.union_sem_class_0) == 0:
            return 0
        else:
            iou_background = (len(self.intersection_class_0) / len(self.union_sem_class_0))
    ...
    def calculate_iou_all_images(self):
        ...
        """
        iou_all_images = ((self.intsec_cl1_total + self.intsec_cl0_total) /
                          (self.union_cl1_total + self.union_cl0_total))
    ...
    def calculate_iou_foreground_all_images(self):
        ...
        """
        iou_foreground_all_images = (self.intsec_cl1_total / self.union_cl1_total)
    ...
    def calculate_iou_background_all_images(self):
        ...
        """
        iou_background = (self.intsec_cl0_total / self.union_cl0_total)
        return iou_background
    ...
    def calculate_mean_iou(self):
        ...
        if len(self.union_sem_class_1) == 0 or len(self.union_sem_class_0) == 0:
            return 0
        else:
            mean_iou = (1 / 2) * ((len(self.intersection_class_1) / len(self.union_sem_class_1)) +
                                  (len(self.intersection_class_0) / len(self.union_sem_class_0)))
            return mean_iou
    ...
    def calculate_mean_iou_all_images(self):
        ...
        mean_iou_all_im = (1 / 2) * ((self.intsec_cl1_total / self.union_cl1_total) +
                                     (self.intsec_cl0_total / self.union_cl0_total))
    ...
    def calculate_pixel_accuracy(self):
        ...
        """
        if len(self.gt_bin_class_1) + len(self.gt_bin_class_0) == 0:
            return 0
        else:
            pixel_accuracy = (len(self.intersection_class_1) + len(self.intersection_class_0)) / (
                    len(self.gt_bin_class_1) + len(self.gt_bin_class_0))
    ...
    def calculate_pixel_accuracy_all_images(self):
        ...
        """
        pixel_accuracy = (self.intsec_cl1_total + self.intsec_cl0_total) / (
                self.gt_cl1_total + self.gt_cl0_total)
        return pixel_accuracy
    ...
    def prepare_eval(self):
        ...
        for i in range(0, len(self.pred_semseg_array)):
            ...
            ignore_gt = self.get_ignore(self.pred_gt_array[i][0])
            ...
            false_positives_class_1 = pred_class_1.difference(self.gt_bin_class_1)
            self.false_pos_cl1_total += len(false_positives_class_1)

            # Calculate the image wise number of pixels that are false positives for the class background
            false_positives_class_0 = pred_class_0.difference(self.gt_bin_class_0)
            self.false_pos_cl0_total += len(false_positives_class_0)

            # Calculate the image wise number of pixels that are false negatives for the class foreground
            false_negatives_class_1 = self.gt_bin_class_1.difference(pred_class_1)
            self.false_neg_cl1_total += len(false_negatives_class_1)

            # Calculate the image wise number of pixels that are false negatives for the class background
            false_negatives_class_0 = self.gt_bin_class_0.difference(pred_class_0)
            self.false_neg_cl0_total += len(false_negatives_class_0)

            # Calculate the image wise IoU for the class foreground
            self.intersection_class_1 = pred_class_1.intersection(self.gt_bin_class_1)
            self.intsec_cl1_total += len(self.intersection_class_1)

            # Calculate the image wise IoU for the class background
            self.intersection_class_0 = pred_class_0.intersection(self.gt_bin_class_0)
            self.intsec_cl0_total += len(self.intersection_class_0)

            # Calculate the image wise Union for the class foreground
            self.union_sem_class_1 = pred_class_1.union(self.gt_bin_class_1)
            self.union_cl1_total += len(self.union_sem_class_1)

            # Calculate the image wise Union for the class background
            self.union_sem_class_0 = pred_class_0.union(self.gt_bin_class_0)
            self.union_cl0_total += len(self.union_sem_class_0)

            # Calculate the image wise Union for the class background
            self.iou_add += self.calculate_iou()
            self.iou_foregrd_add += self.calculate_iou_foreground()
            self.iou_backgrd_add += self.calculate_iou_background()
    ...
    def eval(self):
        ...
        self.iou_foregrd_average = self.iou_foregrd_add / self.length  # IoU_foreground_total
        self.iou_backgrd_average = self.iou_backgrd_add / self.length
        ...
        self.iou_foregrd_total = self.calculate_iou_foreground_all_images()
        self.iou_backgrd_total = self.calculate_iou_background_all_images()
        ...
        self.pixel_accuracy_total = self.calculate_pixel_accuracy_all_images()  # Lösung

        # Compute the average number of true positives for the classes foreground and background as well as their sum
        self.true_pos_foregrd_average = self.intsec_cl1_total / self.length
        self.true_pos_backgrd_average = self.intsec_cl0_total / self.length
        self.true_pos_average = self.true_pos_foregrd_average + self.true_pos_backgrd_average

        # Compute the average number of true positives for all images
        self.true_pos_total = self.intsec_cl1_total + self.intsec_cl0_total

        # Compute the average number of false positives for the classes foreground and background as well as their sum
        self.false_pos_foregrd_average = self.false_pos_cl1_total / self.length
        self.false_pos_backgrd_average = self.false_pos_cl1_total / self.length
        self.false_pos_average = self.false_pos_foregrd_average + self.false_pos_backgrd_average

        # Compute the total number of false positives for all images
        self.false_pos_total = self.false_pos_cl1_total + self.false_pos_cl0_total

        # Compute the average number of false negatives for the classes foreground and background as well as their sum
        self.false_neg_foregrd_average = self.false_neg_cl1_total / self.length
        self.false_neg_backgrd_average = self.false_neg_cl0_total / self.length
        self.false_neg_average = self.false_neg_foregrd_average + self.false_neg_backgrd_average

        # Compute the total number of false negatives for all images
        self.false_neg_total = self.false_neg_cl1_total + self.false_neg_cl0_total
    ...
    def print_iou(self):
        ...
        report_iou = f'Result for the avarage IoU per image: IoU_average={np.round(self.iou_average, decimals=4)}  \n ' \
                     ...
                     f'IoU_average={np.round(self.iou_foregrd_average, decimals=4)}  \n ' \
                     f'Result for the average mean IoU per image for the class background:' \
                     f' IoU_average={np.round(self.iou_backgrd_average, decimals=4)}  \n ' \
                     ...
                     f'IoU_average={np.round(self.iou_foregrd_total, decimals=4)}  \n ' \
                     f'Result for the IoU over all images for the class background: ' \
                     f'IoU_average={np.round(self.iou_backgrd_total, decimals=4)}  \n ' \
    ...
    def print_true_positives(self):
        ...
        report_true_pos = f'Result for the average number of true positives per image for the class foreground :' \
                          f'true_pos_foregrd_average ={np.round(self.true_pos_foregrd_average, decimals=4)}  \n ' \
                          f'Result for the average number of true positives per image for the class background :' \
                          f'true_pos_backgrd_average ={np.round(self.true_pos_backgrd_average, decimals=4)}  \n ' \
                          ...
                          f'true_positives_cl1_total ={np.round(self.intsec_cl1_total, decimals=4)}  \n ' \
                          f'Result for the total number of true positives for the class background :' \
                          f'true_positives_cl0_total ={np.round(self.intsec_cl0_total,decimals =4)}  \n ' \
                          f'Result for the total number of true positives:' \
                          f'true_positives_total =' \
                          f'{np.round(self.intsec_cl1_total + self.intsec_cl0_total, decimals=4)}  \n ' \
    ...
    def print_false_positives(self):
        ...
        report_false_pos = f'Result for the average number of false positives per image for the class foreground :' \
                           f'false_pos_foregrd_average ={np.round(self.false_pos_foregrd_average, decimals=4)}  \n ' \
                           f'Result for the average number of false positives per image for the class background :' \
                           f'false_pos_backgrd_average ={np.round(self.false_pos_backgrd_average, decimals=4)}  \n ' \
                           ...
                           f'false_positives_cl1_total ={np.round(self.false_pos_cl1_total, decimals=4)}  \n ' \
                           f'Result for the total number of false positives for the class background :' \
                           f'false_positives_cl0_total ={np.round(self.false_pos_cl0_total, decimals=4)}  \n ' \
                           f'Result for the total number of true positives:' \
                           f'true_positives_total =' \
                           f'{np.round(self.false_pos_cl1_total + self.false_pos_cl0_total, decimals=4)}  \n ' \
    ...
    def print_false_negatives(self):
        ...
        report_false_neg = f'Result for the average number of false negatives per image for the class foreground :' \
                           f'false_neg_foregrd_average ={np.round(self.false_neg_foregrd_average, decimals=4)}  \n ' \
                           f'Result for the average number of false negatives per image for the class background :' \
                           f'false_neg_backgrd_average ={np.round(self.false_neg_backgrd_average, decimals=4)}  \n ' \
                           ...
                           f'Result for the total number of false positives for the class foreground :' \
                           f'false_negatives_cl1_total ={np.round(self.false_neg_cl1_total, decimals=4)}  \n ' \
                           f'Result for the total number of false positives for the class background :' \
                           f'false_negatives_cl0_total ={np.round(self.false_neg_cl0_total, decimals=4)}  \n ' \
                           f'Result for the total number of false negatives:' \
                           f'false_negatives_total =' \
                           f'{np.round(self.false_neg_cl1_total + self.false_neg_cl0_total, decimals=4)}  \n ' \

        print(report_false_neg)

    def print_additional_information(self):

        add_inf = f'Result for the total number of pixels for all ground truth images that belong to the class ' \
                  f'foreground: gt_class1_total ={np.round(self.gt_cl1_total, decimals=4)}  \n ' \
                  f'Result for the total number of pixels for all ground truth images that belong to the class ' \
                  f'background: gt_class0_total ={np.round(self.gt_cl0_total, decimals=4)}  \n ' \
...
def main():
    ...
    # Generate an array that contains all the names of predicted images by the semantic segmentation net.
    pred_semseg_img_names = semseg.prediction_images_names

    # Generate an array that contains all the predicted images by the semantic segmentation net.
    pred_semseg_img = semseg.prediction_images

    print("Number of images that are used: ", len(pred_semseg_img))

    # Generate an array that contains all the ground truth images.
    pred_gt = semseg.gt_images

    # Generate the array that contains all the names of the ground truth images.
    ...
    # Generate the array that contains all the names of predicted images by the depth estimation net.
    pred_depth_img_names = depth_net.prediction_images_names

    # Generate the array that contains all the predicted images by the depth estimation net.
    pred_depth_images = depth_net.prediction_images

    # Generate the array that contains all the ground truth images that are used by the depth estimation net.
    pred_gt_array = depth_net.gt_images

    # Generate the evaluation for the depth estimation net and print the results
    depth_net_eval = Evaluate(pred_depth_images, pred_gt_array)
    ...
    shared_pred = shared_prediction(pred_semseg_img, pred_depth_images, pred_semseg_img_names,
                                    pred_depth_img_names, pred_gt_array)[0]
    ...
    ground_truth = shared_prediction(pred_semseg_img, pred_depth_images, pred_semseg_img_names,
                                     pred_depth_img_names, pred_gt_array)[1]
